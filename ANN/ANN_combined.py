...
# Prepare data
X_train, X_val, X_test, y_train, y_val, y_test = split_data(combined_wine.drop('quality', axis=1), combined_wine['quality'])
...
torch.save(model.state_dict(), 'wine_quality_model_combined_wine.pth')
...
# Save to CSV
predictions_df.to_csv('data/wine_quality_predictions_combined.csv', index=False)
...
# plt.ylabel('Accuracy (%)')
# plt.title('Prediction Accuracy by Class for Combined Wine')
# plt.xticks(range(10))
# plt.show()