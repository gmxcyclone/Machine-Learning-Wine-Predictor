# white wine only consolidated script
...
red_wine = pd.read_csv('Data/winequality-white_clean.csv')
...
output_size = 10 # Assuming 10 classes for wine quality
hidden_layers = [64, 32]
...
train_losses, val_losses, val_accuracies = train_model(model, criterion, optimizer, train_loader, val_loader, epochs=30)
...
torch.save(model.state_dict(), 'wine_quality_model_white_wine.pth')
...
# Save to CSV
predictions_df.to_csv('data/wine_quality_predictions_white.csv', index=False)
...
plt.ylabel('Accuracy (%)')
plt.title('Prediction Accuracy by Class for White Wine')
plt.xticks(range(10))
plt.show()